...
class Warrior:
    ...
    # TODO, предлагаю передавать только воина.
    def print_info(self, name_warrior, hp):
        if isinstance(name_warrior, Warrior):
            # , стоит переименовать name. Это ведь "другой воин"?
            # , стоит поправить условие.
            #   "другой воин" является Warrior, то стоит отнимать health у него.
            hp -= 20
...
while warrior_1.health > 0 and warrior_2.health > 0:
    ...
    if who_hits == 1:
        # TODO, таким образом передаём имя война и его жизни.
        #  Необходимо передавать объект класса Воин.
        #  К его аргументу health мы сможем обратиться внутри метода print_info =) (name_warrior.health)
